...
function [output,Idk]=Baybayin_word_reader(Mdl, a,Tagalog_words,iniTime,limit)
%tic;
...
text=ocr(v2, 'Language','Tagalog','Language','Malay','TextLayout','Word');
RW  = text.Words;

if isempty(RW)
text=ocr(v2, 'Language','Tagalog','Language','Malay','TextLayout','none');
RW  = text.Words;
if isempty(RW)
...
WBB = text.TextLineBoundingBoxes;
[row, ~]=size(WBB);
%row=1;
...
for i=1:row
    P=WBB(i,:);
    PP=[P(1)-1 P(2)-1 P(3)+1 P(4)+1]; %Workers=12;    
    PPP=imcrop(v2, PP);
...
%======================== Baybayin to Latin Conversion - Word Level ========================%
   ...
   %edits------------------
%    if call1==1
%        character=im2single(character);
%        character=imsegkmeans(character,2); A2=mode(mode(character));
%        character=(character~=A2);
%    end
   %-----------------------
      
      

      P=Baybayin_only(Mdl, character,iniTime,limit);                  %Baybayin OCR System whose output is the equivalent 
...
Q2=cell(QWE_size.^m,1);
        ...
        if m==1     %Syllable Alteration if the input Baybayin word has only one character
           ...
           if strcmp(Q2_new{1},'de') || strcmp(Q2_new{1},'di')
               Q2_new{3}='re'; Q2_new{4}='ri';
           end
           if strcmp(Q2_new{1},'do') || strcmp(Q2_new{1},'du')
